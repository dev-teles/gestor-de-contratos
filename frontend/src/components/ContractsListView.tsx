...
export const ContractsListView: React.FC<ContractsListViewProps> = ({
  ...
  const [supplierMode, setSupplierMode] = useState<'existing' | 'new'>(suppliers.length === 0 ? 'new' : 'existing');
  ...
  useEffect(() => {
    if (suppliers.length === 0) {
      setSupplierMode('new');
    } else if (!newSupplierId && suppliers[0]) {
  ...
  const handleCreateSubmit = (e: React.FormEvent) => {
    ...
    let targetSupplierId = 'sup-padrao';
    let targetSupplierName = 'Fornecedor Cadastrado';
    let targetSupplierCnpj = '00.000.000/0001-00';

    if (supplierMode === 'new' || suppliers.length === 0) {
      const generatedSupId = `sup-${Date.now()}`;
      const cleanName = manualSupplierName.trim() || 'Fornecedor Corporativo';
      const cleanCnpj = manualSupplierCnpj.trim() || '00.000.000/0001-00';
      const newSup: Supplier = {
        ...
        contactEmail: newNotificationEmail.trim() || 'contato@empresa.com.br',
        contactPhone: '(11) 3000-0000',
        activities: ['Serviços & Tecnologia'],
        riskLevel: 'baixo',
        activeContractsCount: 1,
        totalFinancialVolume: val,
        status: 'ativo',
        syncDate: new Date().toLocaleDateString('pt-BR'),
      };
      if (onAddSupplier) {
        onAddSupplier(newSup);
      }
      targetSupplierId = newSup.id;
      targetSupplierName = newSup.razaoSocial;
      targetSupplierCnpj = newSup.cnpj;
    } else {
      const sup = suppliers.find((s) => s.id === newSupplierId) || suppliers[0];
      if (sup) {
        targetSupplierId = sup.id;
        targetSupplierName = sup.razaoSocial;
        targetSupplierCnpj = sup.cnpj;
      }
    }
                  ...
                  <tr>
                    <td colSpan={6} className="py-12 px-4 text-center">
                      <div className="flex flex-col items-center justify-center gap-2">
                        <div className="w-12 h-12 rounded-xl bg-blue-50 flex items-center justify-center text-[#0051d5] mb-1 border border-blue-100">
                          <span className="material-symbols-outlined text-[26px]">
                            {contracts.length === 0 ? 'folder_open' : 'search_off'}
                          </span>
                        </div>
                        ...
                        <p className="text-[12px] text-slate-500 max-w-sm">
                          {contracts.length === 0
                            ? 'A base corporativa está limpa e pronta para novos cadastros. Clique no botão abaixo para adicionar o primeiro instrumento.'
                            : 'Não foram localizados contratos para os filtros aplicados. Tente ajustar os termos de busca ou filtros de status.'}
                        </p>
                        {contracts.length === 0 && currentRole !== 'visualizador' && (
                          <button
                            type="button"
                            onClick={() => setIsDrawerOpen(true)}
                            className="mt-2 h-9 px-4 rounded-xl bg-[#0051d5] hover:bg-[#003ea8] text-white text-[12px] font-semibold flex items-center gap-1.5 shadow-xs transition-all"
                          >
            ...
            <form onSubmit={handleCreateSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
              <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2 p-3 bg-slate-50 rounded-xl border border-slate-200">
                  ...
                  {suppliers.length === 0 || supplierMode === 'new' ? (
                    <div className="flex flex-col gap-2 mt-1">
                      {suppliers.length === 0 && (
                        <p className="text-[11px] text-amber-800 bg-amber-50 p-2.5 rounded-lg border border-amber-200/80 leading-relaxed flex items-center gap-1.5">
                          <span className="material-symbols-outlined text-[16px] text-amber-600 shrink-0">info</span>
                          Nenhum fornecedor cadastrado na base. Preencha os campos abaixo para cadastrá-lo automaticamente junto com o contrato.
                        </p>
                      )}
                      <div className="flex flex-col gap-1">
                        <span className="text-[11px] font-semibold text-slate-700">Razão Social / Nome da Empresa *</span>
                        <input
                          type="text"
                          required
                          value={manualSupplierName}
                          onChange={(e) => setManualSupplierName(e.target.value)}
                          placeholder="Ex: RioMais Tecnologia e Infraestrutura S.A."
                          className="h-10 px-3 rounded-xl bg-white border border-gray-300 text-[13px] text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#0051d5]/20 focus:border-[#0051d5]"
                        />
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="text-[11px] font-semibold text-slate-700">CNPJ do Fornecedor *</span>
                        <input
                          type="text"
                          required
                          ...
                          className="h-10 px-3 rounded-xl bg-white border border-gray-300 text-[13px] text-gray-800 font-mono focus:outline-none focus:ring-2 focus:ring-[#0051d5]/20 focus:border-[#0051d5]"
                        />
                      </div>
                    </div>
                  ) : (
                    <select
                      ...
                      required
                      className="h-10 px-3 rounded-xl bg-white border border-gray-300 text-[13px] text-gray-800 focus:outline-none focus:ring-2 focus:ring-[#0051d5]/20 cursor-pointer"
                    >
                      {suppliers.map((s) => (
                        <option key={s.id} value={s.id}>
