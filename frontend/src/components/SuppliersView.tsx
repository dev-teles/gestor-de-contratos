...
import { receitaFederalService } from '../integrations';
...
export const SuppliersView: React.FC<SuppliersViewProps> = ({
  ...
  const handleConsultarReceita = async () => {
    ...
    try {
      const targetCnpj = cnpj.trim() || '45.109.882/0001-33';
      const result = await receitaFederalService.consultar(targetCnpj);
      const compliance = receitaFederalService.avaliarCompliance(result);

      setCnpj(result.cnpj);
      setRazaoSocial(result.razaoSocial);
      setNomeFantasia(result.nomeFantasia && result.nomeFantasia !== '—' ? result.nomeFantasia : result.razaoSocial);
      if (result.email) setContactEmail(result.email);
      if (result.telefone) setContactPhone(result.telefone);
      if (result.cnaeFiscalDescricao) {
        setSelectedTags((prev) => Array.from(new Set([...prev, result.cnaeFiscalDescricao])));
      }
      setRiskLevel(compliance.nivelRisco);

      showToast(`Dados sincronizados com a Receita Federal (${result.fonteConsulta})! Score: ${compliance.score}/100`);
    } catch (err: unknown) {
      showToast(err instanceof Error ? err.message : 'Erro na consulta à Receita Federal.');
    } finally {
      setIsConsultingReceita(false);
    }
  };
                  ...
                  <tr>
                    <td colSpan={6} className="py-14 px-4 text-center">
                      <div className="flex flex-col items-center justify-center gap-2">
                        <div className="w-12 h-12 rounded-xl bg-blue-50 text-[#0051d5] flex items-center justify-center mb-1 border border-blue-100">
                          <span className="material-symbols-outlined text-[26px]">
                            {suppliers.length === 0 ? 'domain' : 'search_off'}
                          </span>
                        </div>
                        ...
                        <p className="text-[12px] text-slate-500 max-w-sm">
                          {suppliers.length === 0
                            ? 'Sua base de fornecedores e parceiros corporativos está pronta para novos cadastros e homologações.'
                            : 'Nenhum fornecedor corresponde aos filtros ou termos pesquisados.'}
                        </p>
                        {suppliers.length === 0 && currentRole !== 'visualizador' && (
                          <button
                            type="button"
                            onClick={() => setIsDrawerOpen(true)}
